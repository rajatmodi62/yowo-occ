# rm : this does not involve forward pass, 
# can be quickly generated. 

# given a config file, goes to dir where detection txt are dumped 
# generates the fmap for them 
...
args.gtFolder = '/media/yogesh/rm2/occlusion_work/YOWO/evaluation_ucf24_jhmdb/groundtruths_jhmdb'
args.detfolderroot = '/media/yogesh/rm2/occlusion_work/YOWO/backup/jhmdb'


print("set",args.gtFolder)
# exit(1)
...
for p in yaml_paths:
    ...
    p = p.split('/')[-1].split('.')[0]
    p = Path(args.detfolderroot)/p
    args.detFolder = str(p)
    
    if os.path.exists(args.detFolder):
        pass
    else:
        print("contnuing")
        continue
    
    save_path = Path(args.detFolder)/'results'
    save_path.mkdir(exist_ok=True, parents = True)
    args.savePath = save_path
    print("det folder", args.detFolder)
    print("rm",args.gtFolder)
    # exit(1)
    errors = []
    # Validate formats
